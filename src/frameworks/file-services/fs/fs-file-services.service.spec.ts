...
describe('FsFileServices', () => {
  let fileServices: FsFileServices;
  let httpService: HttpService;

  beforeEach(() => {
    httpService = new HttpService();
    fileServices = new FsFileServices(httpService);
    jest
      .spyOn(fileServices, 'getHash')
      .mockResolvedValue(
        '880df782edc9e8afcdcf37fdfa1ee575fb901e4099430c5691fa52f881716946',
      ); // Mock the getHash function directly.
    jest.clearAllMocks();
  });

  describe('retrieveFile', () => {
    it('should retrieve a file from a URL, save it locally, and return the file info including hash', async () => {
      ...
      const mockResponse = { data: avatarBuffer };
      jest
      ...
      const result = await fileServices.retrieveFile(mockUrl);
      ...
      expect(fileServices.getHash).toHaveBeenCalledWith(expect.any(String));
  ...
  describe('removeFile', () => {
    it('should remove a file from the filesystem', async () => {
      const filePath = '/path/to/file.jpg';
      jest.spyOn(fs.promises, 'unlink').mockResolvedValue();

      await fileServices.removeFile(filePath);
