import { Test, TestingModule } from '@nestjs/testing';
import { UserUseCases } from './user.use-case';
import { IDataServices } from '../../core';
import { IRemoteServices } from '../../core/abstracts/remote-services.abstract';
import { IMessageQueueServices } from '../../core';
import { IMailServices } from '../../core/abstracts/mail-services.abstract';
import { IFileServices } from '../../core/abstracts/file-services.abstract';
import { UserFactoryService } from './user-factory.service';
import * as fs from 'fs';
import { NotFoundException } from '@nestjs/common';

jest.mock('fs', () => ({
  readFileSync: jest.fn(),
}));

describe('UserUseCases', () => {
  let userUseCases: UserUseCases;
  let dataServices: IDataServices;
  let remoteServices: IRemoteServices;
  let fileServices: IFileServices;
  let userFactoryService: UserFactoryService;

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        UserUseCases,
        {
          provide: IDataServices,
          useValue: {
            users: { create: jest.fn() },
            avatars: { get: jest.fn(), create: jest.fn(), delete: jest.fn() },
          },
        },
        {
          provide: IRemoteServices,
          useValue: { retrieveUser: jest.fn() },
        },
        {
          provide: IMessageQueueServices,
          useValue: { sendMessage: jest.fn() },
        },
        {
          provide: IMailServices,
          useValue: { sendEmail: jest.fn() },
        },
        {
          provide: IFileServices,
          useValue: { retrieveFile: jest.fn(), removeFile: jest.fn() },
        },
        {
          provide: UserFactoryService,
          useValue: {
            createNewUser: jest.fn(),
            createUserResponse: jest.fn(),
            createNewAvatar: jest.fn(),
          },
        },
      ],
    }).compile();

    userUseCases = module.get<UserUseCases>(UserUseCases);
    dataServices = module.get<IDataServices>(IDataServices);
    remoteServices = module.get<IRemoteServices>(IRemoteServices);
    fileServices = module.get<IFileServices>(IFileServices);
    userFactoryService = module.get<UserFactoryService>(UserFactoryService);
  });

  describe('getUserById', () => {
    it('should return a user response DTO when user is found', async () => {
      const mockUser = {
        id: 100,
        email: 'rachel100@reqres.in',
        firstName: 'Rachel',
        lastName: 'Howell',
        avatarURL: 'https://reqres.in/img/faces/12-image.jpg',
      };
      const mockResponseDto = {
        success: true,
        createdUser: {
          id: 100,
          email: 'rachel100@reqres.in',
          firstName: 'Rachel',
          lastName: 'Howell',
          avatarURL: 'https://reqres.in/img/faces/12-image.jpg',
        },
      };
      jest.spyOn(remoteServices, 'retrieveUser').mockResolvedValue(mockUser);
      jest
        .spyOn(userFactoryService, 'createUserResponse')
        .mockReturnValue(mockResponseDto);

      const result = await userUseCases.getUserById(1);

      expect(remoteServices.retrieveUser).toHaveBeenCalledWith(1);
      expect(userFactoryService.createUserResponse).toHaveBeenCalledWith(
        mockUser,
      );
      expect(result).toEqual(mockResponseDto);
    });
  });
  describe('getUserAvatar', () => {
    const userId = 1;
    const avatar = {
      id: 1,
      hash: 'asdfasdf',
      url: 'https://reqres.in/img/faces/1-image.jpg',
      file: 'C:\\Users\\S3000\\Documents\\Projects\\payever-project\\avatars\\e03e340fcf540cd8a53bf6b51432b4d8.jpg',
    };

    const mockFileContent =
      '';
    const avatarBuffer = Buffer.from(mockFileContent, 'base64'); // Ta-da

    it('should return avatar in base64 when already exists', async () => {
      jest.spyOn(dataServices.avatars, 'get').mockResolvedValue(avatar);
      jest.spyOn(fs, 'readFileSync').mockReturnValue(avatarBuffer);

      const result = await userUseCases.getUserAvatar(userId);

      expect(dataServices.avatars.get).toHaveBeenCalledWith(userId);
      expect(fs.readFileSync).toHaveBeenCalledWith(avatar.file);
      expect(result).toEqual(mockFileContent);
    });

    it('should create and return new avatar if not exists', async () => {
      jest.spyOn(dataServices.avatars, 'get').mockResolvedValue(null);
      const user = {
        success: true,
        createdUser: {
          id: 1,
          avatarURL: 'https://reqres.in/img/faces/1-image.jpg',
          lastName: 'Bluth',
          email: 'george.bluth@reqres.in',
          firstName: 'George',
        },
      };
      const fileData = {
        id: 1,
        hash: '880df782edc9e8afcdcf37fdfa1ee575fb901e4099430c5691fa52f881716946',
        url: 'https://reqres.in/img/faces/1-image.jpg',
        file: 'C:\\Users\\S3000\\Documents\\Projects\\payever-project\\avatars\\e03e340fcf540cd8a53bf6b51432b4d8.jpg',
        __v: 0,
      };
      const mockFileContent =
        '';
      const avatarBuffer = Buffer.from(mockFileContent, 'base64'); // Ta-da
      jest.spyOn(userUseCases, 'getUserById').mockResolvedValue(user);
      jest.spyOn(fileServices, 'retrieveFile').mockResolvedValue(fileData);
      jest.spyOn(userFactoryService, 'createNewAvatar').mockReturnValue(avatar);
      jest.spyOn(dataServices.avatars, 'create').mockResolvedValue(avatar);
      jest.spyOn(fs, 'readFileSync').mockReturnValue(avatarBuffer);

      const result = await userUseCases.getUserAvatar(userId);

      expect(userUseCases.getUserById).toHaveBeenCalledWith(userId);
      expect(fileServices.retrieveFile).toHaveBeenCalledWith(
        user.createdUser.avatarURL,
      );
      expect(userFactoryService.createNewAvatar).toHaveBeenCalledWith(
        userId,
        fileData,
      );
      expect(dataServices.avatars.create).toHaveBeenCalledWith(avatar);
      expect(fs.readFileSync).toHaveBeenCalledWith(avatar.file);
      expect(result).toEqual(
        '',
      );
    });
  });
  describe('deleteUserAvatar', () => {
    it('should delete avatar successfully', async () => {
      const avatar = {
        id: 1,
        hash: '880df782edc9e8afcdcf37fdfa1ee575fb901e4099430c5691fa52f881716946',
        url: 'https://reqres.in/img/faces/1-image.jpg',
        file: 'C:\\Users\\S3000\\Documents\\Projects\\payever-project/avatars/e03e340fcf540cd8a53bf6b51432b4d8.jpg',
        __v: 0,
      };
      jest.spyOn(dataServices.avatars, 'get').mockResolvedValue(avatar);
      jest.spyOn(fileServices, 'removeFile').mockResolvedValue(undefined);
      jest.spyOn(dataServices.avatars, 'delete').mockResolvedValue(undefined);

      await expect(userUseCases.deleteUserAvatar(1)).resolves.toBeUndefined();

      expect(dataServices.avatars.get).toHaveBeenCalledWith(1);
      expect(fileServices.removeFile).toHaveBeenCalledWith(avatar.file);
      expect(dataServices.avatars.delete).toHaveBeenCalledWith(1);
    });

    it('should throw NotFoundException if avatar not found', async () => {
      jest.spyOn(dataServices.avatars, 'get').mockResolvedValue(null);

      await expect(userUseCases.deleteUserAvatar(1)).rejects.toThrow(
        NotFoundException,
      );

      expect(dataServices.avatars.get).toHaveBeenCalledWith(1);
    });
  });
});
